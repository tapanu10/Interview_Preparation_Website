...
const Dbmsdes = () => {
  ...
  const [dbmsData, setdbmsData] = useState(null);
  const [questionAnswer, setQuestionAnswer] = useState({});

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3001/user/fetchQuestion?topic=dbms');
        setdbmsData(response.data.data);
